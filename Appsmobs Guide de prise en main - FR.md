...
Guide de prise en main AppsMobs (pour nouveaux utilisateurs)
> Objectif de ce document
> Te donner, en une seule page, tout ce qu’un nouvel utilisateur doit comprendre pour installer AppsMobs, connecter ses appareils, lancer ses premières automatisations et utiliser le site (compte, abonnement, sécurité).
🧩 1. Comprendre AppsMobs en 5 minutes
AppsMobs, c’est quoi ?
AppsMobs est une application Windows qui te permet de contrôler et automatiser des appareils Android depuis ton PC, avec :
Contrôle en direct via scrcpy (écran du téléphone sur le PC)
Scripts Python prêts pour l’automatisation
Éditeur visuel sans code (blocks)
Vision AI (YOLO) pour détecter des éléments à l’écran et cliquer dessus
Assistant IA pour générer des scripts ou des workflows à partir de texte
Vision globale de l’écosystème
Application de bureau AppsMobs (Electron, Windows uniquement)
Site web AppsMobs (React / Vite) : vitrine, compte utilisateur, abonnement, dashboard en ligne
Backend Auth (Node + Supabase) : gestion des comptes, tokens, sécurité
Base de données : Supabase (utilisée par le site et l’auth)
💻 2. Pré-requis techniques
Côté PC (où AppsMobs tourne)
Windows 10 ou 11, 64 bits
Connexion Internet (recommandée pour l’activation, mises à jour, IA)
Espace disque suffisant (modèles Vision AI, scripts, logs…)
Côté Android
Un ou plusieurs appareils Android
Mode développeur activé
Débogage USB activé
Câble(s) USB stable(s) (éviter les câbles uniquement “charge”)
📥 3. Installation d’AppsMobs sur Windows
Étapes d’installation
Va sur la page Releases de AppsMobs (GitHub Releases ou site officiel).
Télécharge :
soit l’installateur AppsMobs-Setup-x64.exe (recommandé),
soit la version portable AppsMobs-Portable-x64.exe.
Lance le fichier téléchargé :
Suis l’assistant d’installation (NSIS) si tu as pris la version Setup.
Ou décompresse/ouvre simplement si tu utilises la version portable.
Démarre l’application AppsMobs depuis le raccourci ou l’exécutable.
Mises à jour automatiques
L’app intègre electron-updater :
Au démarrage, AppsMobs vérifie si une nouvelle version est dispo sur GitHub.
Si oui, il télécharge et installe la mise à jour (selon ta config).
Côté développeur, la publication se fait via :
npm run release dans le dossier electron-app (pour info, pas nécessaire pour un simple utilisateur).
🔌 4. Première connexion d’un appareil Android
Sur le téléphone
Active Options pour les développeurs.
Active Débogage USB.
Connecte le téléphone au PC via USB et accepte le message “Autoriser le débogage USB”.
Dans AppsMobs
Ouvre AppsMobs.
Va dans la section Connexion / Devices.
AppsMobs détecte automatiquement les appareils via ADB/scrcpy.
Clique sur ton appareil → une fenêtre de streaming s’ouvre (écran du téléphone sur PC).
Tu peux déjà cliquer, swiper, taper du texte depuis l’interface.
Multi-appareils (selon ton plan)
Plan Starter : 1 appareil
Plan Basic : 2 appareils
Plan Pro : 5 appareils
Plan Ultimate : 10 appareils
→ Plus de détails dans la section Tarifs plus bas.
🐍 5. Scripts Python (automation avancée)
Ce que tu peux faire avec les scripts
Lancer des séquences répétitives : farming de récompenses, navigation dans une app, etc.
Automatiser plusieurs appareils en parallèle avec le même script.
Utiliser 100+ fonctions déjà prêtes (tap, swipe, screenshot, vision, etc.).
Exemples de fonctions disponibles
Touch / gestes : tap(x, y), swipe(x1, y1, x2, y2), long_press(x, y)
Temps / attente : wait(5), wait_until_image("button.png")
Écran / image : screenshot(), image_exists("icon.png"), click_image("button.png")
Clavier / navigation : input_text("Bonjour"), press_back(), press_home()
Vision AI (selon plan) : smart_click_yolo(class_name), vision_ai_detect()
Ton premier script (principe général)
Ouvre l’éditeur de scripts dans AppsMobs.
Crée un nouveau script Python.
Utilise les fonctions fournies (avec l’aide de la doc intégrée et de l’auto-complétion).
Associe le script à un ou plusieurs appareils.
Clique sur Run pour exécuter.
🧩 6. Éditeur “No-Code” (Blocks)
Pour qui ?
Pour les utilisateurs qui ne veulent pas coder en Python.
Pour créer vite des automations simples (taper, cliquer, attendre, conditions, boucles…).
Principe de fonctionnement
Tu drag & drop des blocs (actions, conditions, boucles).
Chaque bloc correspond à une fonction de script (tap, swipe, etc.).
Tu peux voir la logique de ton workflow comme un schéma.
Optionnel : exporter ou transformer ce workflow en script Python pour aller plus loin.
👁️ 7. Vision AI (YOLO) – Automatiser “à la vue”
(Disponible sur certains plans, voir tarifs)
À quoi ça sert ?
Tu apprends à AppsMobs à reconnaître des boutons, icônes, récompenses sur l’écran Android.
Au lieu de cliquer à des coordonnées fixes ou avec des selecteurs fragiles, tu dis :
“Clique quand tu vois ce bouton” (détection par IA).
Utile quand :
L’UI change souvent.
Il n’y a pas d’API.
C’est un jeu ou une app très graphique.
Fonctionnement général
Tu fournis des captures d’écran.
Tu annotes les éléments à reconnaître.
AppsMobs entraîne un modèle YOLO.
Dans tes scripts, tu appelles smart_click_yolo("nom_de_classe") ou équivalent.
L’IA détecte la position sur l’écran et AppsMobs clique automatiquement.
🤖 8. Assistant IA (Gemini, Groq, OpenAI, Claude, DeepSeek…)
Rôle de l’assistant IA
Tu décris en langage naturel ce que tu veux faire :
“Ouvre l’app X, attends 5 secondes, clique sur le bouton vert, puis fais un swipe vers le haut toutes les 30 secondes.”
L’assistant génère :
Soit un script Python,
Soit un workflow en blocs.
Configuration
Il fonctionne en BYOK (Bring Your Own Key) :
Gemini, Groq, OpenAI, Claude, DeepSeek…
Tu renseignes tes propres clés API dans la configuration de l’app.
Important : Ne jamais partager tes clés, et ne pas les exposer dans du code public.
⏰ 9. Tâches planifiées & pool de devices
Planificateur de tâches
Tu peux dire à AppsMobs :
“Lance ce script toutes les 5 minutes”
ou “Lance ce script tous les jours à 3h du matin”.
Pratique pour :
Farming auto,
Tests récurrents,
Surveillance d’apps.
Device pool
Tu attribues des appareils à des tâches.
Le scheduler se charge de lancer les bons scripts sur les bons appareils, dans le bon ordre.
🌐 10. Site web AppsMobs (compte, abonnement, dashboard en ligne)
Technos (pour info)
Frontend : React 18 + Vite + Tailwind CSS
Backend auth : Node.js + Express
Auth / DB : Supabase
...
Création de compte / connexion (via Supabase)
Gestion des abonnements & paiements :
Stripe, PayPal, Binance (selon intégration)
Dashboard web :
Voir tes licences, ton statut d’abonnement
Accéder à des pages de documentation
Support & Chat IA :
Assistance, FAQ, parfois chat IA intégré pour les questions fréquentes
💳 11. Tarifs & limites de plan (résumé)
Tableau simplifié
Starter (≈ 9 € / mois)
1 appareil
Scripts illimités
...
10 appareils
Scripts illimités
Vision AI + entraînement Vision RL
Support prioritaire (≈ 24 h)
> Remarque : Les prix exacts et promos peuvent changer → se référer au site appsmobs.com.
🔐 12. Sécurité & bonnes pratiques utilisateur
Même si beaucoup de protections sont en place côté appli et serveur, voici ce que toi, utilisateur, dois retenir :
Compte & authentification
Utilise une adresse email valide et un mot de passe fort :
Longueur suffisante
Majuscules, minuscules, chiffres, caractères spéciaux
Ne partage jamais ton compte ou ton mot de passe.
Déconnecte-toi des sessions que tu n’utilises plus, surtout sur des PC partagés.
Tokens & sessions
L’app et le site utilisent des tokens sécurisés (validation, expiration).
Si tu vois un comportement étrange (déconnexion répétée, erreurs 401/403), reconnecte-toi proprement.
Évite de manipuler les tokens à la main (localStorage, cookies…) si tu n’es pas développeur.
Paiements
Les paiements passent par Stripe / PayPal / Binance, via des pages sécurisées (HTTPS).
Vérifie toujours que tu es bien sur le site officiel (URL correcte, HTTPS).
Clés API IA
Tes clés (Gemini, OpenAI, etc.) sont sensibles :
Ne les copies jamais dans du code public.
Ne les partages pas à d’autres utilisateurs.
Si tu penses qu’une clé a fuité, régénère-la dans le dashboard du fournisseur (Google, OpenAI, etc.).
Mises à jour
Laisse AppsMobs se mettre à jour régulièrement :
Correctifs de bugs
Nouvelles fonctionnalités
Corrections de sécurité
🆘 13. Où trouver de l’aide ?
Documentation en ligne
Docs officielles : docs.appsmobs.com
Tutoriels & guides d’exemples : voir les sections “Documentation” et “Support” sur le site
Support
Formulaire ou email : support@appsmobs.com
Selon le plan : communauté, email 48h, ou prioritaire
Ressources utiles
Site officiel : https://appsmobs.com
Téléchargements / Releases : https://github.com/AppsMobs/AppsMobs/releases
✅ 14. Résumé rapide pour démarrer
Installer AppsMobs sur Windows 10/11 (Setup ou Portable).
Activer le débogage USB sur ton Android et le connecter au PC.
Ouvrir AppsMobs, vérifier que l’appareil apparaît et que l’écran s’affiche.
Essayer un premier script ou un workflow en blocks (actions simples : tap, wait, swipe).
Si ton plan le permet, tester Vision AI sur un cas simple (détection d’un bouton).
Créer / gérer ton compte et ton abonnement sur le site AppsMobs.
Garder l’app à jour et respecter les bonnes pratiques de sécurité.
Si tu veux, je peux maintenant transformer ce contenu en un fichier ONBOARDING_UTILISATEUR.md prêt à être ajouté à ton repo, ou l’adapter pour un public plus “développeur interne” plutôt que “utilisateur final”.
redige le en md document

Guide de prise en main AppsMobs (pour nouveaux utilisateurs)
> Objectif
> Ce document regroupe tout ce qu’un nouvel utilisateur doit comprendre pour installer AppsMobs, connecter ses appareils, lancer ses premières automatisations et utiliser le site (compte, abonnement, sécurité).
🧩 1. Comprendre AppsMobs en 5 minutes
AppsMobs, c’est quoi ?
AppsMobs est une application Windows qui te permet de contrôler et automatiser des appareils Android depuis ton PC, avec :
Contrôle en direct via scrcpy (écran du téléphone sur le PC)
...
Mode développeur activé
Débogage USB activé
Câble(s) USB stables (éviter les câbles uniquement “charge”)
📥 3. Installation d’AppsMobs sur Windows
Étapes d’installation
Va sur la page Releases de AppsMobs (GitHub Releases ou site officiel).
Télécharge :
...
Mises à jour automatiques
L’app intègre electron-updater :
Au démarrage, AppsMobs vérifie si une nouvelle version est disponible.
Si oui, il télécharge et installe la mise à jour.
Tu n’as rien à faire de spécial : laisser l’app se mettre à jour est recommandé.
🔌 4. Première connexion d’un appareil Android
Sur le téléphone
Active Options pour les développeurs.
Active Débogage USB.
...
Dans AppsMobs
Ouvre AppsMobs.
Va dans la section de gestion des devices.
AppsMobs détecte automatiquement les appareils via ADB / scrcpy.
Clique sur ton appareil → une fenêtre de streaming s’ouvre (écran du téléphone sur PC).
Tu peux déjà cliquer, swiper, taper du texte depuis l’interface.
Multi-appareils (selon ton plan)
...
Écran / image : screenshot(), image_exists("icon.png"), click_image("button.png")
Clavier / navigation : input_text("Bonjour"), press_back(), press_home()
Vision AI (selon plan) : smart_click_yolo("classe"), vision_ai_detect()
Ton premier script (principe général)
Ouvre l’éditeur de scripts dans AppsMobs.
Crée un nouveau script Python.
Utilise les fonctions fournies (avec la doc intégrée et l’auto-complétion).
Associe le script à un ou plusieurs appareils.
Clique sur Run pour exécuter.
🧩 6. Éditeur “No-Code” (Blocks)
Pour qui ?
Pour les utilisateurs qui ne veulent pas coder en Python.
Pour créer rapidement des automations simples (taper, cliquer, attendre, conditions, boucles…).
Principe de fonctionnement
Tu drag & drop des blocs (actions, conditions, boucles).
Chaque bloc correspond à une fonction de script (tap, swipe, etc.).
Tu peux voir la logique de ton workflow comme un schéma.
Tu peux ensuite, si besoin, exporter ce workflow en script Python pour aller plus loin.
👁️ 7. Vision AI (YOLO) – Automatiser “à la vue”
(Disponible sur certains plans, voir tarifs)
À quoi ça sert ?
Tu apprends à AppsMobs à reconnaître des boutons, icônes, récompenses sur l’écran Android.
Au lieu de cliquer à des coordonnées fixes, tu dis :
“Clique quand tu vois ce bouton” (détection IA).
Utile quand :
L’UI change souvent.
Il n’y a pas d’API fiable.
C’est un jeu ou une app très graphique.
Fonctionnement général
Tu fournis des captures d’écran.
Tu annotes les éléments à reconnaître (bounding boxes, classes).
AppsMobs entraîne un modèle YOLO.
Dans tes scripts, tu appelles par exemple smart_click_yolo("nom_de_classe").
L’IA détecte la position sur l’écran et AppsMobs clique automatiquement.
🤖 8. Assistant IA (Gemini, Groq, OpenAI, Claude, DeepSeek…)
Rôle de l’assistant IA
Tu décris en langage naturel ce que tu veux faire :
...
L’assistant génère :
un script Python, ou
un workflow en blocs.
Configuration
L’assistant fonctionne en BYOK (Bring Your Own Key) :
Gemini, Groq, OpenAI, Claude, DeepSeek…
Tu renseignes tes propres clés API dans la configuration.
Important : ne jamais exposer tes clés dans du code public ou des captures d’écran.
⏰ 9. Tâches planifiées & pool de devices
Planificateur de tâches
Tu peux demander à AppsMobs :
“Lance ce script toutes les 5 minutes”
ou “Lance ce script tous les jours à 3h du matin”.
Très utile pour :
Farming automatique,
...
Surveillance d’apps.
Device pool
Tu définis un pool d’appareils.
Tu assignes des tâches à ces appareils.
Le scheduler se charge de lancer les bons scripts sur les bons appareils, dans le bon ordre.
🌐 10. Site web AppsMobs (compte, abonnement, dashboard en ligne)
Fonctionnalités principales côté utilisateur
...
Support & Chat IA :
Assistance, FAQ, chat IA pour certaines questions
Pour info (technique)
Frontend : React 18 + Vite + Tailwind CSS
Backend auth : Node.js + Express
Auth / DB : Supabase
...
Vision AI + entraînement Vision RL
Support prioritaire (≈ 24 h)
> Note : Les prix exacts et les remises peuvent varier. Toujours vérifier sur le site officiel appsmobs.com.
🔐 12. Sécurité & bonnes pratiques utilisateur
Compte & authentification
Utilise une adresse email valide et un mot de passe fort.
...
Validés (format, expiration),
Nettoyés automatiquement s’ils sont invalides.
En cas de comportement étrange (souvent déconnecté, erreurs 401/403), reconnecte-toi proprement.
Paiements
Les paiements passent par des plateformes sécurisées (Stripe, PayPal, Binance).
Vérifie toujours l’URL (HTTPS, domaine correct).
Clés API IA
Ne partage jamais tes clés (Gemini, OpenAI, etc.).
Si tu suspectes une fuite, régénère la clé depuis le dashboard du fournisseur.
Mises à jour
Laisse AppsMobs se mettre à jour régulièrement :
Correctifs de bugs
Nouvelles fonctionnalités
...
Documentation
Docs officielles : https://docs.appsmobs.com
Guides, FAQ, exemples d’automations (voir le site AppsMobs)
Support
Email : support@appsmobs.com
Type de support selon ton plan (communauté, email classique, prioritaire)
Ressources utiles
Site officiel : https://appsmobs.com
Téléchargements / Releases : https://github.com/AppsMobs/AppsMobs/releases
✅ 14. Résumé ultra-rapide pour démarrer
Installer AppsMobs sur Windows 10/11 (Setup ou Portable).
Activer le débogage USB sur ton Android et le connecter au PC.
Ouvrir AppsMobs, vérifier que l’appareil apparaît et que l’écran s’affiche.
Créer un premier script ou workflow en blocks (tap, wait, swipe).
Tester, puis éventuellement activer Vision AI si ton plan le permet.
Gérer ton compte et ton abonnement via le site AppsMobs.
Garder l’application à jour et respecter les bonnes pratiques de sécurité.